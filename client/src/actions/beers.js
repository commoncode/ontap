...
// vote for a beer
export function voteForBeer(beerId) {
  dispatcher.dispatch({
    type: 'REQUEST_VOTE_FOR_BEER',
    beerId,
  });

  return fetcher(`/api/v1/beers/${beerId}/vote`, {
    method: 'PUT',
  })
  .then((data) => {
    addNotification('Vote added.');
    dispatcher.dispatch({
      type: 'RECEIVE_VOTE_FOR_BEER',
      beerId,
      data,
    });
  })
  .catch((error) => {
    if (error.isClean && error.code === 401) {
      addNotification('Please log in to vote for beers');
    }
...
// un-vote for a beer
export function unvoteForBeer(beerId) {
  dispatcher.dispatch({
    type: 'REQUEST_UNVOTE_FOR_BEER',
    beerId,
  });
  ...
  .then((data) => {
    addNotification('Removed your vote.');
    dispatcher.dispatch({
      type: 'RECEIVE_UNVOTE_FOR_BEER',
      beerId,
      data,
    });
  })
...
// clear all votes on a beer
export function clearVotes(beerId) {
  dispatcher.dispatch({
    type: 'REQUEST_CLEAR_VOTES',
    beerId,
  });

  return fetcher(`/api/v1/beers/${beerId}/clearvotes/`, {
    method: 'POST',
  })
  .then(() => {
    addNotification('Votes cleared. ');
    dispatcher.dispatch({
      type: 'RECEIVE_CLEAR_VOTES',
      beerId,
    });
  })
  .catch((error) => {
